<x-layout.app>
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <p class="text-emerald-600 mt-2">{{ __('Submit your complaint and we\'ll work to resolve it as soon as possible.') }}</p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-emerald-100 p-6">
            <form action="{{ route('complaints.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <!-- File Attachment -->
                <div class="mb-8">
                    <label for="file" class="block text-sm font-medium text-emerald-700 mb-2">
                        {{ __('Attach Supporting Document (Optional)') }}
                    </label>
                    <div class="flex items-center justify-center w-full">
                        <label for="file" class="flex flex-col items-center justify-center w-full h-32 border-2 border-emerald-300 border-dashed rounded-lg cursor-pointer bg-emerald-50 hover:bg-emerald-100 transition-colors">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                <svg class="w-8 h-8 mb-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path>
                                </svg>
                                <p class="mb-2 text-sm text-emerald-700"><span class="font-semibold">{{ __('Click to upload') }}</span> {{ __('or drag and drop') }}</p>
                                <p class="text-xs text-emerald-600">{{ __('JPG, PNG, PDF, DOC (Max: 10MB)') }}</p>
                            </div>
